...
1. **The "net cost" figures are a steady-state-BME identity — and the CIP footnote confirms the split.** The CIP body states **90.25% of burn → app + validator operators, 9.75% net**, and a **footnote** decomposes the 9.75% as **5% Canton Foundation dev fund (CIP-0082) + 4.75% Super Validators** (5% + 4.75% = 9.75%; 90.25% = 0.95 × 0.95): *"These numbers are the stable emissions starting ten years from network launch."* (The footnote is in the live Google Doc; it did not survive into the plain-text copy in this repo — which is why an earlier pass thought it absent.) Because Burn-Mint Equilibrium means total mint ≈ total burn at steady state, "for every $1 burned, ~90¢ returns to operators" holds *at that equilibrium*, and §7's distribution-override is precisely what lets a synchronizer operator capture that 90.25% for its own activity. Two caveats are spec-editorial, not blockers: (a) these are the **stable emissions ~10 years post-launch** — today's split is SV-heavy and still migrating, so 9.75% net is a forward-looking floor, not today's reality; (b) BME equality is an *equilibrium* property (the CIP itself notes the network is currently inflationary, i.e. mint > burn — which makes net economics *better* than 9.75%), not a per-transaction protocol guarantee. So the table's "Net" columns are sound — just label them as steady-state economics.
...
| 8 | Stake 20%; per-round shortfall auto-burn (no rewards); replenish-or-forfeit | `LockedAmulet`/`TimeLock`; CIP-0105 lifecycle shape; mining-round clock | **Realized-TPS oracle** (hardest); **coupon-free** metric-gated partial burn of locked principal — the `splitAndBurn` path mints a `ValidatorRewardCoupon`, so this is a new choice (upstream); per-round trigger |
...
- **GATE-0 — Net-cost framing (resolved).** The 90.25%/9.75% split is a steady-state-BME identity (valid ~10 yr out), so the table's Net columns are sound. A CIP **footnote** confirms the 9.75% = 5% dev fund (CIP-0082) + 4.75% SV ("stable emissions ~10 yr from launch"). No longer a blocker. Actions: implement minting as the curve (not a fixed % of burn) and label the Net columns as *steady-state* (today's split is SV-heavy/migrating; the network is currently inflationary, so net is better than 9.75% now). The live tokenomics gate is GATE-1.
...
1. **Net-cost framing (GATE-0):** the footnote's decomposition of the 9.75% net — **5% dev fund (CIP-0082) + 4.75% SV**, "stable emissions ~10 yr from launch" — is clear. Please just label the table's Net / Discounted-Net columns as *steady-state* (~10 yr) economics (today's split is SV-heavy and migrating; the network is currently inflationary, so net is better than 9.75% today). **Open (GATE-1):** is §7 report-driven minting *additive* to the issuance curve or *drawn from* it? (Determines whether this changes the 100B/decade cap and BME semantics network-wide.)
